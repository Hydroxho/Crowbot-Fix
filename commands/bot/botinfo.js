...
module.exports = {
	...
	run: async (client, message, args, prefix, color) => {
		...
		if (client.config.owner.includes(message.author.id)) {
            ...
            embed.setURL('https://discord.gg/purpleshop')
            embed.setDescription('**Une description complète et détaillée du Bot** <@1041361148102070352>')
            embed.setColor(color)
            .setThumbnail(message.author.avatarURL({ dynamic:true }));
            embed.setTimestamp()
            embed.setFooter(`${client.config.name}`)
            embed.addFields(
                { name: '👑 Owner / Developer :', value: '<@520271784205877248>', inline: true },
                ...
                { name: '📞 Shop :', value: `[Clique ICI Pour Rejoindre Mon Shop](https://discord.gg/purpleshop)`, inline: true },
